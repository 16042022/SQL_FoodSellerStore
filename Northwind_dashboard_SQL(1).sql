...
-- TRa ve tong doanh thu cac quoc gia tai cung ky 2015, 2016--

DECLARE @halfQrt2016 date;
DECLARE @halfQrt2015 date;

SET @halfQrt2016 = (Select MAX(OrderDate) from Orders);
SET @halfQrt2015 = DATEADD(YEAR, -1, @halfQrt2016);

with rev2016 as 
(
	SELECT ShipCountry, SUM((OrderDetails.UnitPrice - (OrderDetails.UnitPrice*OrderDetails.Discount))*OrderDetails.Quantity) AS rev_2016
	FROM (select * from Orders WHERE OrderDate >= '2016-01-01' and OrderDate <= @halfQrt2016) tt2016_
	JOIN OrderDetails on tt2016_.OrderID = OrderDetails.OrderID
	Group by ShipCountry
),

rev_2015 as
(
	SELECT ShipCountry, SUM((OrderDetails.UnitPrice - (OrderDetails.UnitPrice*OrderDetails.Discount))*OrderDetails.Quantity) AS rev_2015
	FROM (select * from Orders WHERE OrderDate >= '2015-01-01' and OrderDate <= @halfQrt2015) tt2015_
	JOIN OrderDetails on tt2015_.OrderID = OrderDetails.OrderID
	Group by ShipCountry
)

select r1.ShipCountry, r1.rev_2015, r2.rev_2016
from rev_2015 r1
left join rev2016 r2 on r1.ShipCountry = r2.ShipCountry
order by r1.ShipCountry;
...
DECLARE @halfQrt2016_1 date;
DECLARE @halfQrt2015_1 date;
SET @halfQrt2016_1 = (Select MAX(OrderDate) from Orders);
SET @halfQrt2015_1 = DATEADD(YEAR, -1, @halfQrt2016_1);
...
on o1.OrderID = od1.OrderID
where (o1.OrderDate >= '2015-01-01' and o1.OrderDate <= @halfQrt2015_1) 
OR (o1.OrderDate >= '2016-01-01' and o1.OrderDate <= @halfQrt2016_1) 
...
on p1.CategoryID = cate1.CategoryID
where cate1.CategoryName in ('Dairy Products','Beverages','Confections');
...
DECLARE @halfQrt2016_2 date;
DECLARE @halfQrt2015_2 date;
SET @halfQrt2016_2 = (Select MAX(OrderDate) from Orders);
SET @halfQrt2015_2 = DATEADD(YEAR, -1, @halfQrt2016_2);

with employee_rev_2015 as (select 
	o1.EmployeeID, 
	sum((od1.UnitPrice-(od1.UnitPrice*od1.Discount))*od1.Quantity) as revenue_2015
from Northwind.dbo.Orders o1
inner join Northwind.dbo.OrderDetails od1
on o1.OrderID = od1.OrderID
where OrderDate >= '2015-01-01' AND OrderDate <= @halfQrt2015_2
...
inner join Northwind.dbo.OrderDetails od1
on o1.OrderID = od1.OrderID
where OrderDate >= '2016-01-01' AND OrderDate <= @halfQrt2016_2
group by o1.EmployeeID)
...
select 
	...
	, er15.revenue_2015
	, er16.revenue_2016
from employee_rev_2015 er15
left join employee_rev_2016 er16 on er15.EmployeeID = er16.EmployeeID
join Employees e1 on er15.EmployeeID = e1.EmployeeID;
...
DECLARE @halfQrt2016_3 date;
DECLARE @halfQrt2015_3 date;
SET @halfQrt2016_3 = (Select MAX(OrderDate) from Orders);
SET @halfQrt2015_3 = DATEADD(YEAR, -1, @halfQrt2016_3);
...
bill_2015 as (select 
	...
	and OrderDate >= '2015-01-01' AND OrderDate <= @halfQrt2015_3) as bill_2015,
	sum(total_bill) + (select sum(total_bill)
						from hr_management hr, Raw_1 r1
						where hr.EmployeeID = r1.EmployeeID
						and hr.manager_level > 0
						and (OrderDate >= '2015-01-01' AND OrderDate <= @halfQrt2015_3)) as final_bill_2015
from hr_management hr
join Raw_1 r1
on hr.EmployeeID = r1.EmployeeID
where manager_level = 0
and OrderDate >= '2015-01-01' AND OrderDate <= @halfQrt2015_3
...
select 
	...
	sum(total_bill) + (select sum(total_bill)
						...
						where hr.EmployeeID = r1.EmployeeID
						and hr.manager_id = (select EmployeeID from hr_management where Title like '%manager%')
						and OrderDate >= '2015-01-01' AND OrderDate <= @halfQrt2015_3) as final_bill_2014
from hr_management hr
join Raw_1 r1
on hr.EmployeeID = r1.EmployeeID
...
from bill_2015 b2015, bill_2016 b2016
where b2015.EmployeeID = b2016.EmployeeID;
